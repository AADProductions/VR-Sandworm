...
CGINCLUDE
	...
	float4 _LightProperties;//xyz = position w = intensity
...
//		half4 Sky = tex2D(_Skybox, i.uv);
		...
		float3 sunDir = normalize(-_LightParams.xyz);
...
//		return fogFac; // for debugging
		...
		float4 boost = (_LightParams.w * _ColBoost) ;
		if(Scattering)
			//fogCol = (((_LightColor0 + boost * boost)*mPhase)*_Intensity)+unity_FogColor;
			fogCol.xyz = fogCol+sunEGround*boost;
